...
#	real data from colorectal cancer MSI/MSS example
...
# read in data
tmp<-read.table('GSE13294_series_matrix_TLR4_updated2.txt',header=T, row.names=1, sep="\t", colClasses=c('character','character',rep('numeric',47)))
# define y and x
y0<-as.numeric(as.factor(tmp[,1]))
x0<-data.frame(tmp[,-1])
...
# for each run do CPE ...
		kk<-bn
		for(kk in bn:(lst2-1)){
			clda <- lda(Y ~ ., data=x, prior = c(1,1)/2, subset = c(1:kk))
...
# for each run do CPE ...
		...
		Y2<-as.factor(y2)
		for(kk in bn:(lst2-1)){
			logc<-glm(y2 ~ ., data=x, family="binomial", subset = c(1:kk))
...
# for each run do CPE ...
		kk<-bn
		for(kk in bn:(lst2-1)){
			cknn<-knn(x[1:kk,],x[1:kk,], cl=Y[1:kk], k=K)
...
# for each run do CPE ...
		...
		Ysub<-Y[1:kk]
		for(kk in bn:(lst2-1)){
			ckknn<-kknn(Ysub ~ ., train=Xsub, test=x[kk+1,], k=K, distance=Dist, kernel=Kernel)
			predv[[i]]$kknn[j,kk-bn+1]<-fitted.values(ckknn)
			err[[i]]$kknn[j,kk-bn+1]<-(abs(predv[[i]]$kknn[j,kk-bn+1]-y[kk+1]))^powerr			
		}
		fval[[i]]$kknn[j,]<-fitted.values(kknn(Ysub ~ ., train=Xsub, test=x[1:kk,], k=K, distance=Dist, kernel=Kernel))
...
# save workspace
filen<-paste("ch4computing_real_cld2_",seed,".RData",sep="")
save.image(file=filen)
# repeat the above several times for different random seeds, save in different workspaces
# then load and compile results across saved workspaces

# key components are predv, err, CPE
.libPaths("/Library/Frameworks/R.framework/Versions/Current/library")
load('ch4computing_real_cld2_2115.RData')
predv1<-predv
err1<-err
fval1<-fval
CPE1<-CPE
load('ch4computing_real_cld2_3115.RData')
predv2<-predv
err2<-err
fval2<-fval
CPE2<-CPE
load('ch4computing_real_cld2_4115.RData')
predv3<-predv
err3<-err
fval3<-fval
CPE3<-CPE
load('ch4computing_real_cld2_5115.RData')
predv4<-predv
err4<-err
fval4<-fval
CPE4<-CPE
...
#
# save workspace as ch4class_real_set2.RData
#
.libPaths("/Library/Frameworks/R.framework/Versions/Current/library")
load('ch4class_real_set2.RData')
...
plot(seq(60,155,10),CPEres[1,],type='b',pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,27),xlab="Sample size", ylab="CPE", main="CPE by model type")
lines(seq(60,155,10),CPEres[2,],type='b',pch="2")
lines(seq(60,155,10),CPEres[3,],type='b',pch="3")
lines(seq(60,155,10),CPEres[4,],type='b',pch="4")
legend('topleft',legend=c('lda','logc','knn','kknn'),pch=c("1","2","3","4"),lty=1,cex=1.3)
# save as ch4class_set2_CPE_bw_v2.pdf

CPEres1<-CPEres
for(i in 1:dim(CPEres)[1]){ CPEres1[i,]<-CPEres[i,]-CPEres[i,1]}
plot(seq(60,155,10),CPEres1[1,],type='b',pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,27),xlab="Sample size", ylab="CPE", main="CPE by model type (baseline adjusted)")
lines(seq(60,155,10),CPEres1[2,],type='b',pch="2")
lines(seq(60,155,10),CPEres1[3,],type='b',pch="3")
lines(seq(60,155,10),CPEres1[4,],type='b',pch="4")
legend('topleft',legend=c('lda','logc','knn','kknn'),pch=c("1","2","3","4"),lty=1,cex=1.3)
# save as ch4class_set2_CPEadj_bw_v2.pdf

plot(seq(60,155,10),CPEres[1,],type='n',pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,27),xlab="Sample size", ylab="CPE", main="CPE by model type")
polygon(c(rev(nval),nval),c(rev(CPEres[1,]+CPEresSE[1,]),CPEres[1,]-CPEresSE[1,]),col='grey15',border=NA)
lines(seq(60,155,10),CPEres[1,])
points(seq(60,155,10),CPEres[1,],pch="1")
...
lines(seq(60,155,10),CPEres[2,])
points(seq(60,155,10),CPEres[2,],pch="2")
...
lines(seq(60,155,10),CPEres[3,])
points(seq(60,155,10),CPEres[3,],pch="3")
...
lines(seq(60,155,10),CPEres[4,])
points(seq(60,155,10),CPEres[4,],pch="4")
lines(nval,CPEres[4,]+CPEresSE[4,],lty='dashed',col='red')
lines(nval,CPEres[4,]-CPEresSE[4,],lty='dashed',col='red')
legend('topleft',legend=c('lda','logc','knn','kknn'),pch=c("1","2","3","4"),lty=1,cex=1.3)
# save as ch4class_set2_CPEsd_v2.pdf
