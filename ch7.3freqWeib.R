...
save.image('ch7computing_freqW_31122.RData')
#save.image('ch7computing_freqW_21122.RData')
#save.image('ch7computing_freqW_21112.RData')
#save.image('ch7computing_freqW_21012.RData')
#save.image('ch7computing_freqW_10010.RData')

## integrating results across permutations; load each workspace, save FPE results
# 23 runs
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqW_10010.RData")
FPE1w<-FPE
modPH1w<-modPH
medPH1w<-medPH
# 20 runs each
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqW_21012.RData")
FPE2w<-FPE
modPH2w<-modPH
medPH2w<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqW_21112.RData")
FPE3w<-FPE
modPH3w<-modPH
medPH3w<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqW_21122.RData")
FPE4w<-FPE
modPH4w<-modPH
medPH4w<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqW_31122.RData")
FPE5w<-FPE
modPH5w<-modPH
medPH5w<-medPH
# 
...
npred<-80
#
FPEavgfw<-c(rep(0,npred))
for(i in 1:nperm2) FPEavgfw<-FPEavgfw+FPE1w[[i]]$freq
for(i in 1:nperm2) FPEavgfw<-FPEavgfw+FPE2w[[i]]$freq
for(i in 1:nperm2) FPEavgfw<-FPEavgfw+FPE3w[[i]]$freq
for(i in 1:nperm2) FPEavgfw<-FPEavgfw+FPE4w[[i]]$freq
for(i in 1:nperm2) FPEavgfw<-FPEavgfw+FPE5w[[i]]$freq
FPEavgfw<-FPEavgfw/nperm
FPEsdfw<-c(rep(0,npred))
for(i in 1:nperm2) FPEsdfw<-FPEsdfw+(FPEavgfw-FPE1w[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfw<-FPEsdfw+(FPEavgfw-FPE2w[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfw<-FPEsdfw+(FPEavgfw-FPE3w[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfw<-FPEsdfw+(FPEavgfw-FPE4w[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfw<-FPEsdfw+(FPEavgfw-FPE5w[[i]]$freq)^2
FPEsdfw<-sqrt(FPEsdfw/(nperm-1))
plot(1:npred,FPE1w[[1]]$freq, type="l", xlab='time step', ylab='L1 cumulative predictive error')
lines(1:npred,FPE2w[[2]]$freq, type="l", col='red') 
lines(1:npred,FPEavgfw, type="l", col='blue')
lines(1:npred,FPEavgfw-2*FPEsdfw, type="l", lty=2, col='green')
lines(1:npred,FPEavgfw+2*FPEsdfw, type="l", lty=2, col='green')
