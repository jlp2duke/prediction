...
# scenario 1
# 1) separated normals in the plane

## run using different seeds, save results from each in separate workspace
#seed<-1054
#seed<-1099
#seed<-1215
seed<-2115
...
# generate dataset
	...
	M <- t(chol(sigma))
	Z1 <- matrix(rnorm(nval[length(nval)]/2,mean=2),2,nval[length(nval)]/2) # 2 rows, nval[length(nval)]/2 columns
   	X1 <- t(M %*% Z1)
	Z2 <- matrix(rnorm(nval[length(nval)]/2,mean=-2),2,nval[length(nval)]/2) # 2 rows, nval[length(nval)]/2 columns
   	X2 <- t(M %*% Z2)
   	X<-NULL
   	y<-NULL
   	for(i in 1:(nval[length(nval)]/2)){
		X <- rbind(X,X1[i,],X2[i,])
		y <- c(y,1,2)
	}
   	Y <- as.factor(y)
...
# save workspace
filen<-paste("ch4computing_cld1_",seed,".RData",sep="")
...
load('ch4computing_cld1_1054.RData')
predv1<-predv
err1<-err
fval1<-fval
CPE1<-CPE
load('ch4computing_cld1_1099.RData')
predv2<-predv
err2<-err
fval2<-fval
CPE2<-CPE
load('ch4computing_cld1_1215.RData')
predv3<-predv
err3<-err
fval3<-fval
CPE3<-CPE
load('ch4computing_cld1_2115.RData')
predv4<-predv
err4<-err
fval4<-fval
CPE4<-CPE
...
#
# save workspace as ch4class_sim_cld1.RData
#
.libPaths("/Library/Frameworks/R.framework/Versions/Current/library")
load('ch4class_sim_cld1.RData')
...
plot(seq(20,200,10),CPEres[1,],type='b',pch="1",cex.lab=1.2, cex.axis=1.2, ylim=c(0,1.2),xlab="Sample size", ylab="CPE", main="CPE by model type")
...
# save as ch4class_cd1_CPE_bw_v2.pdf

CPEres1<-CPEres
for(i in 1:dim(CPEres)[1]){ CPEres1[i,]<-CPEres[i,]-CPEres[i,1]}
plot(seq(20,200,10),CPEres1[1,],type='b',pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,1.2),xlab="Sample size", ylab="CPE", main="CPE by model type (baseline adjusted)")
...
legend('topleft',legend=c('lda','logc','knn','kknn'),pch=c("1","2","3","4"),lty=1,cex=1.3)
# save as ch4class_cd1_CPEadj_bw_v2.pdf

plot(seq(20,200,10),CPEres[1,],type='n', pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,1.2),xlab="Sample size", ylab="CPE", main="CPE by model type")
...
lines(nval,CPEres[4,]-CPEresSE[4,],lty='dashed',col='red')
legend('topleft',legend=c('lda','logc','knn','kknn'),pch=c("1","2","3","4"),lty=1,cex=1.3)
# save as ch4class_cd1_CPEerr_v2.pdf
