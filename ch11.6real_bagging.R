...
for(k in 1:(ny-nyb)){
	...
	for(kk in 1:nclust){
		c0<-datn2[1:(nloc*(nyb2+1)),c(1:43,46,47)]
		c1<-c0[which(cmem==kk),]
		c1<-as.data.frame(c1)
		...
		#[1] 1200  47
			...
			## with ensembling: bagging - trees
				...
				c2 <- as.data.frame(scale(c0, center = mins, scale = maxs - mins))
				tree3t<-train(SSG0_6~., data=c2, method="treebag", metric="RMSE", keepX=TRUE, nbagg=10, trControl=trControl) 
				tmod[[k]][[kk]]<-tree3t
				c2<-c1[c1$Year==as.numeric(Year[nyb2+1]),]
				sc2 <- as.data.frame(scale(c2, center = mins, scale = maxs - mins))
				out<-predict(tree3t, sc2)
				tpred[[k]][[kk]]<-cbind(c2$SSG0_6,out*(max(c0$SSG0_6, na.rm=T)-min(c0$SSG0_6, na.rm=T))+min(c0$SSG0_6, na.rm=T))
			## with ensembling bagging- neural net
				c0<-c1[c1$Year<as.numeric(Year[nyb2+1]),]
				maxs <- apply(c0, 2, max, na.rm=T) 
				mins <- apply(c0, 2, min, na.rm=T)
				c2 <- as.data.frame(scale(c0, center = mins, scale = maxs - mins))
				nn3t<-train(SSG0_6~., data=c2, method="avNNet", tuneGrid=mygrid2[1:3,], linout=TRUE, maxit=100, trace=F) 
				nnmod[[k]][[kk]]<-nn3t
				out<-predict(nn3t, sc2)				
				nnpred[[k]][[kk]]<-out*(max(c0$SSG0_6, na.rm=T)-min(c0$SSG0_6, na.rm=T))+min(c0$SSG0_6, na.rm=T)
			}
		}
	cat(kk, "of ", nclust, "\r") 
	flush.console()
