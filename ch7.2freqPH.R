...
#save.image('ch7computing_freqPH_31122.RData')
save.image('ch7computing_freqPH_21122.RData')
#save.image('ch7computing_freqPH_21112.RData')
#save.image('ch7computing_freqPH_21012.RData')
#save.image('ch7computing_freqPH_10010.RData')

## integrating results across permutations; load each workspace, save FPE results
# 23 runs
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPH_10010.RData")
FPE1<-FPE
modPH1<-modPH
medPH1<-medPH
# 20 runs each
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPH_21012.RData")
FPE2<-FPE
modPH2<-modPH
medPH2<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPH_21112.RData")
FPE3<-FPE
modPH3<-modPH
medPH3<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPH_21122.RData")
FPE4<-FPE
modPH4<-modPH
medPH4<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPH_31122.RData")
FPE5<-FPE
modPH5<-modPH
medPH5<-medPH
# 
...
for(i in 1:nperm2) FPEavgf<-FPEavgf+FPE1[[i]]$freq
for(i in 1:nperm2) FPEavgf<-FPEavgf+FPE2[[i]]$freq
for(i in 1:nperm2) FPEavgf<-FPEavgf+FPE3[[i]]$freq
for(i in 1:nperm2) FPEavgf<-FPEavgf+FPE4[[i]]$freq
for(i in 1:nperm2) FPEavgf<-FPEavgf+FPE5[[i]]$freq
FPEavgf<-FPEavgf/nperm
FPEsdf<-c(rep(0,npred))
for(i in 1:nperm2) FPEsdf<-FPEsdf+(FPEavgf-FPE1[[i]]$freq)^2
for(i in 1:nperm2) FPEsdf<-FPEsdf+(FPEavgf-FPE2[[i]]$freq)^2
for(i in 1:nperm2) FPEsdf<-FPEsdf+(FPEavgf-FPE3[[i]]$freq)^2
for(i in 1:nperm2) FPEsdf<-FPEsdf+(FPEavgf-FPE4[[i]]$freq)^2
for(i in 1:nperm2) FPEsdf<-FPEsdf+(FPEavgf-FPE5[[i]]$freq)^2
FPEsdf<-sqrt(FPEsdf/(nperm-1))
plot(1:npred,FPE1[[1]]$freq, type="l", xlab='time step', ylab='L1 cumulative predictive error')
lines(1:npred,FPE2[[2]]$freq, type="l", col='red') 
lines(1:npred,FPEavgf, type="l", col='blue')
lines(1:npred,FPEavgf-2*FPEsdf, type="l", lty=2, col='green')
lines(1:npred,FPEavgf+2*FPEsdf, type="l", lty=2, col='green')

plot(1:npred,FPE1[[1]]$freq, type="l", xlab='time step', ylab='L1 cumulative predictive error', col='grey75', cex.lab=1.2, cex.axis=1.2, cex=1.2, ylim=c(0,17000))
lines(1:npred,FPE2[[2]]$freq, type="l", col='grey75') 
lines(1:npred,FPEavgf, type="l", col='black')
lines(1:npred,FPEavgf-2*FPEsdf, type="l", lty=2)
lines(1:npred,FPEavgf+2*FPEsdf, type="l", lty=2)
