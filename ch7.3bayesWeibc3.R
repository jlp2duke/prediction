...
# save workspace
filen<-paste("ch7computing_survBayes_wt",seed,".RData",sep="")
save.image(file=filen)

## integrating results across permutations; load each workspace, save FPE results
# 23 runs
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_wt10010.RData")
FPE1bwc3<-FPE
modPH1bwc3<-modPH
medPH1bwc3<-medPH
# 20 runs each
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_wt21012.RData")
FPE2bwc3<-FPE
modPH2bwc3<-modPH
medPH2bwc3<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_wt21112.RData")
FPE3bwc3<-FPE
modPH3bwc3<-modPH
medPH3bwc3<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_wt21122.RData")
FPE4bwc3<-FPE
modPH4bwc3<-modPH
medPH4bwc3<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_wt31122.RData")
FPE5bwc3<-FPE
modPH5bwc3<-modPH
medPH5bwc3<-medPH
# 
...
nperm<-100
#
FPEavgbwc3<-c(rep(0,npred))
for(i in 1:nperm2) FPEavgbwc3<-FPEavgbwc3+FPE1bwc3[[i]]$bayes2
for(i in 1:nperm2) FPEavgbwc3<-FPEavgbwc3+FPE2bwc3[[i]]$bayes2
for(i in 1:nperm2) FPEavgbwc3<-FPEavgbwc3+FPE3bwc3[[i]]$bayes2
for(i in 1:nperm2) FPEavgbwc3<-FPEavgbwc3+FPE4bwc3[[i]]$bayes2
for(i in 1:nperm2) FPEavgbwc3<-FPEavgbwc3+FPE5bwc3[[i]]$bayes2
FPEavgbwc3<-FPEavgbwc3/nperm
FPEsdbwc3<-c(rep(0,npred))
for(i in 1:nperm2) FPEsdbwc3<-FPEsdbwc3+(FPEavgbwc3-FPE1bwc3[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbwc3<-FPEsdbwc3+(FPEavgbwc3-FPE2bwc3[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbwc3<-FPEsdbwc3+(FPEavgbwc3-FPE3bwc3[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbwc3<-FPEsdbwc3+(FPEavgbwc3-FPE4bwc3[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbwc3<-FPEsdbwc3+(FPEavgbwc3-FPE5bwc3[[i]]$bayes2)^2
FPEsdbwc3<-sqrt(FPEsdbwc3/(nperm-1))
plot(1:npred,FPE1bwc3[[1]]$bayes2, type="l", xlab='time step', ylab='L1 cumulative predictive error')
lines(1:npred,FPE2bwc3[[2]]$bayes2, type="l", col='red') 
lines(1:npred,FPEavgbwc3, type="l", col='blue')
lines(1:npred,FPEavgbwc3-2*FPEsdbwc3, type="l", lty=2, col='green')
lines(1:npred,FPEavgbwc3+2*FPEsdbwc3, type="l", lty=2, col='green')
# bayCPE_c3_Weib.pdf
