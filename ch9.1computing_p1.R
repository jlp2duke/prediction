#			ch9.1 computing
#16 explanatory variables, eps ~ N(0,?), Xs iid ~ N(0,?)
#true model?
# y = logX1 + sqrt(X2) + X3 + X4^2 + log(X1)*sqrt(X2) + sqrt(X2)*X3 +X3*X4^2 + X4*log(X1) + eps
# sd = 5
# eps ~ N(0,5/2)
#start with 50 data pt burn in
#use forward/backward
#	best subset
#	AIC
#	BIC
#	BIC using n^(1/4) instead of log(n)
#use model selection, get 5 models, 5 predictors
#predict first group of 5, for each find SSE of prediction
#repeat above using 55 data pt to fit model, repeat, ...
#no permutation
#repeat above 
# histogram of model terms selected by each approach
# second model: log, square root, linear, quadratic, ..., i.e.,
# y = 10log(X1) + 9sqrt(X2) + 8X3 + 7(X4)^2 + ... + 1X10 + X11/1 + X12/2 + ... + X20/10 + eps
# rectangular terms? but same number of terms
# y = 10X1 + 9X2 + 8X1X2 + 7X4 + 6X5 + 5X4X5 + 4X7 + ... + 1X10 + X11/1 + X12/2 + ... + X20/10 + eps

#second simulation to redo up to model selection from above with 10 explanatory
#variables and 5 in model to get multidimensional histogram on model terms, then do
#MDS to get one point per model term and logical structure
...
# as individual plots
hist(termStep, breaks=seq(0.5,nvar+1.5,by=1), xlim=c(1,nvar+1),ylim=c(0,100), xlab="term", main="Term frequency selection for stepwise: best")
hist(termSub, breaks=seq(0.5,nvar+1.5,by=1),xlim=c(1,nvar+1),ylim=c(0,100), xlab="term", main="Term frequency selection for best subsets")
hist(termAIC, breaks=seq(0.5,nvar+1.5,by=1),xlim=c(1,nvar+1),ylim=c(0,100), xlab="term", main="Term frequency selection for AIC")
hist(termBIC, breaks=seq(0.5,nvar+1.5,by=1),xlim=c(1,nvar+1),ylim=c(0,100), xlab="term", main="Term frequency selection for BIC")
hist(termN, breaks=seq(0.5,nvar+1.5,by=1),xlim=c(1,nvar+1),ylim=c(0,100), xlab="term", main="Term frequency selection for n^(1/2)")
