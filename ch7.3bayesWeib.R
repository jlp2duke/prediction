...
#in the treatment group (TREAT = 1) who is 60 years old. For a given time
#t,the probability that this individual survives beyond t days is given by
#P(T>t)=exp(−exp(z)), where z = (log t − μ − β1(1)−β2(60))/σ
...
# save workspace
#filen<-paste("ch7computing_survBayes_w",seed,".RData",sep="")
#save.image(file=filen)

## integrating results across permutations; load each workspace, save FPE results
# 23 runs
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_w10010.RData")
FPE1bw<-FPE
modPH1bw<-modPH
medPH1bw<-medPH
# 20 runs each
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_w21012.RData")
FPE2bw<-FPE
modPH2bw<-modPH
medPH2bw<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_w21112.RData")
FPE3bw<-FPE
modPH3bw<-modPH
medPH3bw<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_w21122.RData")
FPE4bw<-FPE
modPH4bw<-modPH
medPH4bw<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_survBayes_w31122.RData")
FPE5bw<-FPE
modPH5bw<-modPH
medPH5bw<-medPH
# 
...
nperm<-100
#
FPEavgbw<-c(rep(0,npred))
for(i in 1:nperm2) FPEavgbw<-FPEavgbw+FPE1bw[[i]]$bayes2
for(i in 1:nperm2) FPEavgbw<-FPEavgbw+FPE2bw[[i]]$bayes2
for(i in 1:nperm2) FPEavgbw<-FPEavgbw+FPE3bw[[i]]$bayes2
for(i in 1:nperm2) FPEavgbw<-FPEavgbw+FPE4bw[[i]]$bayes2
for(i in 1:nperm2) FPEavgbw<-FPEavgbw+FPE5bw[[i]]$bayes2
FPEavgbw<-FPEavgbw/nperm
FPEsdbw<-c(rep(0,npred))
for(i in 1:nperm2) FPEsdbw<-FPEsdbw+(FPEavgbw-FPE1bw[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbw<-FPEsdbw+(FPEavgbw-FPE2bw[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbw<-FPEsdbw+(FPEavgbw-FPE3bw[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbw<-FPEsdbw+(FPEavgbw-FPE4bw[[i]]$bayes2)^2
for(i in 1:nperm2) FPEsdbw<-FPEsdbw+(FPEavgbw-FPE5bw[[i]]$bayes2)^2
FPEsdbw<-sqrt(FPEsdbw/(nperm-1))
plot(1:npred,FPE2bw[[1]]$bayes2, type="l", xlab='time step', ylab='L1 cumulative predictive error')
lines(1:npred,FPE2bw[[2]]$bayes2, type="l", col='red') 
lines(1:npred,FPEavgbw, type="l", col='blue')
lines(1:npred,FPEavgbw-2*FPEsdbw, type="l", lty=2, col='green')
lines(1:npred,FPEavgbw+2*FPEsdbw, type="l", lty=2, col='green')
# bayCPE_n80_Weib.pdf

## load workspaces for both bayes and freq results, original data
nval<-(nbase+1):nobs
plot(nval,FPEavgfw, type='n', ylim=c(0,16000), xlab='sample size', ylab='L1 cumulative predictive error')
polygon(c(rev(nval),nval),c(rev(FPEavgfw+FPEsdfw),FPEavgfw-FPEsdfw),col=rgb(89/255,89/255,89/255,0.8),border=NA)
lines(nval,FPEavgfw)
lines(nval,FPEavgfw+FPEsdfw,lty='dashed',col='red')
lines(nval,FPEavgfw-FPEsdfw,lty='dashed',col='red')
polygon(c(rev(nval),nval),c(rev(FPEavgbw+FPEsdbw),FPEavgbw-FPEsdbw),col=rgb(153/255,153/255,153/255,0.8),border=NA)
lines(nval,FPEavgbw,col='blue')
lines(nval,FPEavgbw+FPEsdbw,lty='dashed',col='green')
lines(nval,FPEavgbw-FPEsdbw,lty='dashed',col='green')

% CPE with +/- SD

plot(nval,FPEavgfw, type='n', ylim=c(0,16000), xlab='sample size', ylab='L1 cumulative predictive error', cex.axis=1.2, cex.lab=1.2, cex=1.2)
#polygon(c(rev(nval),nval),c(rev(FPEavgfw+FPEsdfw),FPEavgfw-FPEsdfw),col=rgb(89/255,89/255,89/255,0.8),border=NA)
lines(nval,FPEavgfw)
lines(nval,FPEavgfw+FPEsdfw,lty='dashed')
lines(nval,FPEavgfw-FPEsdfw,lty='dashed')
polygon(c(rev(nval),nval),c(rev(FPEavgbw+FPEsdbw),FPEavgbw-FPEsdbw),col=rgb(153/255,153/255,153/255,0.8),border=NA)
lines(nval,FPEavgbw,lty='dotted')
legend('topleft',legend=c('freq CPE','freq +/- SD','bayes CPE', 'bayes +/- SD'),col=c('black',rgb(89/255,89/255,89/255,0.8),'black',rgb(153/255,153/255,153/255,0.8)),lty=c(1,2,3,1),lwd=c(1,1,1,5),cex=1.2)
# save as avgCPE-orig-Weib_v2.pdf

plot(nval,FPEavgfwc1, type='n', ylim=c(0,16000), xlab='sample size', ylab='L1 cumulative predictive error', cex.axis=1.2, cex.lab=1.2, cex=1.2)
lines(nval,FPEavgfwc1)
lines(nval,FPEavgfwc1+FPEsdfwc1,lty='dashed')
lines(nval,FPEavgfwc1-FPEsdfwc1,lty='dashed')
polygon(c(rev(nval),nval),c(rev(FPEavgbwc1+FPEsdbwc1),FPEavgbwc1-FPEsdbwc1),col=rgb(153/255,153/255,153/255,0.8),border=NA)
lines(nval,FPEavgbwc1,lty='dotted')
legend('topleft',legend=c('freq CPE','freq +/- SD','bayes CPE', 'bayes +/- SD'),col=c('black','black','black',rgb(153/255,153/255,153/255,0.8)),lty=c(1,2,3,1),lwd=c(1,1,1,5),cex=1.2)
# save as avgCPE-c1-Weib_v2.pdf

plot(nval,FPEavgfwc3, type='n', ylim=c(0,16000), xlab='sample size', ylab='L1 cumulative predictive error', cex.axis=1.2, cex.lab=1.2, cex=1.2)
lines(nval,FPEavgfwc3)
lines(nval,FPEavgfwc3+FPEsdfwc3,lty='dashed')
lines(nval,FPEavgfwc3-FPEsdfwc3,lty='dashed')
polygon(c(rev(nval),nval),c(rev(FPEavgbwc3+FPEsdbwc3),FPEavgbwc3-FPEsdbwc3),col=rgb(153/255,153/255,153/255,0.8),border=NA)
lines(nval,FPEavgbwc3,lty='dotted')
legend('topleft',legend=c('freq CPE','freq +/- SD','bayes CPE', 'bayes +/- SD'),col=c('black','black','black',rgb(153/255,153/255,153/255,0.8)),lty=c(1,2,3,1),lwd=c(1,1,1,5),cex=1.2)
# save as avgCPE-c1-Weib_v3.pdf

plot(nval,FPEavgfw, type='n', ylim=c(0,16000), xlab='sample size', ylab='L1 cumulative predictive error', cex.axis=1.2, cex.lab=1.2, cex=1.2)
lines(nval,FPEavgfw)
lines(nval,FPEavgbw,lty=2)
lines(nval,FPEavgfwc1,lty=3)
lines(nval,FPEavgbwc1,lty=4)
lines(nval,FPEavgfwc3,lty=5,col=rgb(153/255,153/255,153/255,0.8))
lines(nval,FPEavgbwc3,lty=6,col=rgb(153/255,153/255,153/255,0.8))
legend('topleft',legend=c('freq orig','bayes orig','freq c1','bayes c1','freq c3','bayes c3'),lty=c(1:6),col=c(rep('black',4),rep(rgb(153/255,153/255,153/255,0.8),2)),cex=1.2)
# save as avgCPE-all-Weib_v2.pdf