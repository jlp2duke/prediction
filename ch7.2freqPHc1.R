...
save.image('ch7computing_freqPHc_31122.RData')
#save.image('ch7computing_freqPHc_21122.RData')
#save.image('ch7computing_freqPHc_21112.RData')
#save.image('ch7computing_freqPHc_21012.RData')
#save.image('ch7computing_freqPHc_10010.RData')

## integrating results across permutations; load each workspace, save FPE results
# 23 runs
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPHc_10010.RData")
FPE1c1<-FPE
modPH1c1<-modPH
medPH1c1<-medPH
# 20 runs each
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPHc_21012.RData")
FPE2c1<-FPE
modPH2c1<-modPH
medPH2c1<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPHc_21112.RData")
FPE3c1<-FPE
modPH3c1<-modPH
medPH3c1<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPHc_21122.RData")
FPE4c1<-FPE
modPH4c1<-modPH
medPH4c1<-medPH
load("/Users/jclarke/Documents/predictive/Book/ch7/uissurv/ch7computing_freqPHc_31122.RData")
FPE5c1<-FPE
modPH5c1<-modPH
medPH5c1<-medPH
# remove lots of stuff, then save as ch7computing_freqPHc1.RData
nperm<-100
nperm2<-20
npred<-80
#
FPEavgfc1<-c(rep(0,npred))
for(i in 1:nperm2) FPEavgfc1<-FPEavgfc1+FPE1c1[[i]]$freq
for(i in 1:nperm2) FPEavgfc1<-FPEavgfc1+FPE2c1[[i]]$freq
for(i in 1:nperm2) FPEavgfc1<-FPEavgfc1+FPE3c1[[i]]$freq
for(i in 1:nperm2) FPEavgfc1<-FPEavgfc1+FPE4c1[[i]]$freq
for(i in 1:nperm2) FPEavgfc1<-FPEavgfc1+FPE5c1[[i]]$freq
FPEavgfc1<-FPEavgfc1/nperm
FPEsdfc1<-c(rep(0,npred))
for(i in 1:nperm2) FPEsdfc1<-FPEsdfc1+(FPEavgfc1-FPE1c1[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfc1<-FPEsdfc1+(FPEavgfc1-FPE2c1[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfc1<-FPEsdfc1+(FPEavgfc1-FPE3c1[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfc1<-FPEsdfc1+(FPEavgfc1-FPE4c1[[i]]$freq)^2
for(i in 1:nperm2) FPEsdfc1<-FPEsdfc1+(FPEavgfc1-FPE5c1[[i]]$freq)^2
FPEsdfc1<-sqrt(FPEsdfc1/(nperm-1))
plot(1:npred,FPE1c1[[1]]$freq, type="l", xlab='time step', ylab='L1 cumulative predictive error', col='grey75', cex.lab=1.2, cex.axis=1.2, cex=1.2, ylim=c(0,17000))
lines(1:npred,FPE2c1[[2]]$freq, type="l", col='grey75') 
lines(1:npred,FPEavgfc1, type="l", col='black')
lines(1:npred,FPEavgfc1-2*FPEsdfc1, type="l", lty=2)
lines(1:npred,FPEavgfc1+2*FPEsdfc1, type="l", lty=2)
# save as freqCPE-n80-PH_v2.pdf
