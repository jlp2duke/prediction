...
#	ii. all four variables in model (ch4computing_cld2m2.R)
...
# for each run do CPE ...
		kk<-bn
		for(kk in bn:(lst2-1)){
			clda <- lda(Y ~ x1+x2+x12+x22, data=x, prior = c(1,1)/2, subset = c(1:kk))
...
# for each run do CPE ...
		...
		Y2<-as.factor(y2)
		for(kk in bn:(lst2-1)){
			logc<-glm(y2 ~ x1+x2+x12+x22, data=x, family="binomial", subset = c(1:kk))
...
# for each run do CPE ...
		...
		Ysub<-Y[1:kk]
		for(kk in bn:(lst2-1)){
			ckknn<-kknn(Ysub ~ x1+x2+x12+x22, train=Xsub, test=x[kk+1,], k=K, distance=Dist, kernel=Kernel)
...
# save workspace
filen<-paste("ch4computing_cld2m2_",seed,".RData",sep="")
...
load('ch4computing_cld2m2_1054.RData')
predv1<-predv
err1<-err
fval1<-fval
CPE1<-CPE
load('ch4computing_cld2m2_1099.RData')
predv2<-predv
err2<-err
fval2<-fval
CPE2<-CPE
load('ch4computing_cld2m2_1215.RData')
predv3<-predv
err3<-err
fval3<-fval
CPE3<-CPE
load('ch4computing_cld2m2_2115.RData')
predv4<-predv
err4<-err
fval4<-fval
CPE4<-CPE
...
#
# save workspace as ch4class_sim_cld2m2.RData
#
.libPaths("/Library/Frameworks/R.framework/Versions/Current/library")
load('ch4class_sim_cld2m2.RData')
...
# plot results
plot(seq(20,200,10),CPEres[1,],type='b',pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,52),xlab="Sample size", ylab="CPE", main="CPE by model type")
...
# save as ch4class_cd2m2_CPE_bw_v2.pdf

CPEres1<-CPEres
for(i in 1:dim(CPEres)[1]){ CPEres1[i,]<-CPEres[i,]-CPEres[i,1]}
plot(seq(20,200,10),CPEres1[3,],type='b',pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,50),xlab="Sample size", ylab="CPE", main="CPE by model type (baseline adjusted)")
lines(seq(20,200,10),CPEres1[2,],type='b',pch="2")
lines(seq(20,200,10),CPEres1[1,],type='b',pch="3")
lines(seq(20,200,10),CPEres1[4,],type='b',pch="4")
legend('topleft',legend=c('lda','logc','knn','kknn'),pch=c("1","2","3","4"),lty=1,cex=1.3)
# save as ch4class_cd2m2_CPEadj_bw.pdf

plot(seq(20,200,10),CPEres[1,],type='n',pch="1",cex.lab=1.2, cex.axis=1.2,ylim=c(0,52),xlab="Sample size", ylab="CPE", main="CPE by model type")
...
lines(nval,CPEres[4,]-CPEresSE[4,],lty='dashed',col='red')
legend('topleft',legend=c('lda','logc','knn','kknn'),pch=c("1","2","3","4"),lty=1,cex=1.3)
# save as ch4class_cd2m2_CPEerr_v2.pdf
